                ...
                {planesDisponibles.map((plan, index) => (
                    <div
                        ...
                        className={classNames(
                            ...
                            !isLastChild(planesDisponibles, index) && 'border-r-0 xl:border-r border-gray-200 dark:border-gray-700',
                        )}
                    >
                        <div>
                            <h5 className="mb-6 flex items-center gap-2">
                                <span>{plan.nombre}</span>
                                {plan.recomendado && <Tag className="rounded-full bg-green-200 font-bold">Recomendado</Tag>}
                            </h5>
                            <div>{plan.descripcion}</div>
                            <div className="mt-6">
                                <NumericFormat className="h1" displayType="text" value={plan.precio[ciclopago]} prefix="$" thousandSeparator />
                                <span className="text-lg font-bold"> / {ciclopago === 'mensual' ? 'mes' : 'año'}</span>
                            </div>
                            <div className="flex flex-col gap-4 border-t border-gray-200 dark:border-gray-700 mt-6 pt-6">
                                {listaCaracteristicas.map((caracteristica) =>
                                    plan.caracteristicas.includes(caracteristica.id) && (
                                        <div key={caracteristica.id} className="flex items-center gap-4 font-semibold heading-text">
                        ...
                        <div className="mt-10">
                            <Button
                                block
                                onClick={() => {
                                    setPlanSeleccionado({ ciclopago, nombrePlan: plan.nombre, precio: plan.precio })
                                    setDialogoPago(true)
                                }}
                            >
                                Seleccionar plan
